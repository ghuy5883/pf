...
public:
    ...
    SIMDVector3 LengthSq_v() const
    {
        // TODO Lab 01g
        /*
        Length Squared: Equivalent to the dot product of the same vector together.
                        
                        Dot product(VectorA, VectorA): 
                        (A.x * A.x) + (A.y * A.y) + (A.z * A.z) 
        */
        SIMDVector3 temp = Dot_v(*this, *this);
        return temp;
    }
    ...
    {
        // TODO Lab 01g
        SIMDVector3 temp = LengthSq_v();
        return temp.mVec.m128_f32[0];
    }
    ...
    SIMDVector3 Length_v() const
    {
        // TODO Lab 01g
        /*
        Length: Can be achieved by taking the square root of the result from
                LengthSquared.
        */
        SIMDVector3 temp = LengthSq_v();
        temp.mVec = _mm_sqrt_ps(temp.mVec);
        return temp;
    }
    ...
    void Normalize()
    {
        // TODO Lab 01g
        /*
        Normalize:  Can be achieved by dividing the target vector by its magnitude (length).
                    
                    Division of the vector can be achieved by inverting the value of the
                    magnitude (1/magnitude) and multiplying the target vector by that new
                    value.
        */
        SIMDVector3 temp = LengthSq_v();
        temp.mVec = _mm_rsqrt_ps(temp.mVec);
        mVec = _mm_mul_ps(mVec, temp.mVec);
        return;
    }
    ...
    friend SIMDVector3 Dot_v(const SIMDVector3& a, const SIMDVector3& b)
    {
        // TODO Lab 01g
        /*
        Dot Product:    Can be achieved using the SSE dot product function. The
                        mask is 0x77 --> 0111 0111 to indicate x, y, z values for 
                        the input and x, y, z values written for the output.
        */
        __m128 temp = _mm_dp_ps(a.mVec, b.mVec, 0x77);
    ...
    {
        // TODO Lab 01g
        __m128 temp = _mm_dp_ps(a.mVec, b.mVec, 0x71);
    ...
    friend SIMDVector3 Cross(const SIMDVector3& a, const SIMDVector3& b)
    {
        // TODO Lab 01g
        /*
        Cross Product:  Achieved using SSE shuffle to write new vector C such that
                        C = ( <Ay, Az, Ax> * <Bz, Bx, By>) – (<Az, Ax, Ay> * <By, Bz, Bx>).
        */
        ...
        __m128 lhs = _mm_mul_ps(tempA, tempB);

        tempA = _mm_shuffle_ps(a.mVec, a.mVec, _MM_SHUFFLER(2, 0, 1, 3));
        tempB = _mm_shuffle_ps(b.mVec, b.mVec, _MM_SHUFFLER(1, 2, 0, 3));
        __m128 rhs = _mm_mul_ps(tempA, tempB);

       __m128 tempC = _mm_sub_ps(lhs, rhs);

        return SIMDVector3(tempC);
    }
    ...
    friend SIMDVector3 Lerp(const SIMDVector3& a, const SIMDVector3& b, float f)
    {
        // TODO Lab 01g
        /*
        Lerp:   VectorA + constant_value(VectorB - VectorA);
        */
        __m128 temp = _mm_sub_ps(b.mVec, a.mVec);
        __m128 rhs = _mm_mul_ps(temp, _mm_set_ps1(f));
        __m128 retVal = _mm_add_ps(a.mVec, rhs);
        return SIMDVector3(retVal);
    }
...
public:
    ...
    SIMDVector4 LengthSq_v() const
    {
        ...
        Length Squared: Equivalent to the dot product of the same vector together.
                        ...
                        (A.x * A.x) + (A.y * A.y) + (A.z * A.z)
        */
        SIMDVector4 temp = Dot_v(*this, *this);
        return temp;
    }
    ...
    void Normalize()
    {
        // TODO Lab 01h
        /*
        Normalize:  Using dot product with both inputs as the same vector results in
                    a vector with the original x, y, z values squared.

                    Take the square root of each of the new vector's values, then invert
                    the values. The new values (fractional) can then be multiplied by the original 
                    vector for a divisional effect.
        */
        SIMDVector4 temp = LengthSq_v();
        temp.mVec = _mm_rsqrt_ps(temp.mVec);
        mVec = _mm_mul_ps(mVec, temp.mVec);
    ...
    friend SIMDVector4 Dot_v(const SIMDVector4& a, const SIMDVector4& b)
    {
        ...
        /*
        Dot Product:    Can be achieved using the SSE dot product function. The
                        mask is 0xFF --> 1111 1111 to indicate x, y, z, w values for
                        the input and x, y, z, w values written for the output.
        */
        __m128 temp = _mm_dp_ps(a.mVec, b.mVec, 0xFF);
        return SIMDVector4(temp);
    }
    ...
    {
        // TODO Lab 01h
        __m128 temp = _mm_dp_ps(a.mVec, b.mVec, 0xF1);
        return temp.m128_f32[0];
    }
